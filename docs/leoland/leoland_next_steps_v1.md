# Leoland Next Steps v1

## What Leoland is right now

Leoland is currently:

- mostly a prototype shell
- partly an engine experiment
- partly a seed for a richer leadership/society simulation

It is not yet a playable world-state game.

## Best next bounded tranche

The best next move is:

1. runtime cleanup and first playable shell
2. replace Finland placeholder data with first Leoland world-state schema
3. define first region/province and party structure
4. introduce a real turn loop based on Leoland canon rather than example decisions

## Strong recommendation

Do not jump straight into many mechanics.

First build:

- first world-state schema
- first Leoland data seed
- first turn loop that expresses the values/principles already captured

## Likely later sequence

- L2: runtime cleanup / first playable shell
- L3: world-state schema / first simulation turn
- L4: football/league sibling repo intake
